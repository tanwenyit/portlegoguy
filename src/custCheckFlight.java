...
class custCheckFlight extends JFrame {
    JFrame frame;
    ArrayList<Flight> flightList;
    JScrollPane scrollboard;    // enable the result to be scrollable
    
    public custCheckFlight(Flight search) throws IOException {
        ...
        frame = new JFrame("Flight Schedule");
        ...
        for(int i=0 ; i < flightList.size() ; i++){ // for search result
            if(flightList.get(i).getDep().equals(search.getDep()) && flightList.get(i).getDes().equals(search.getDes())){
                if(dateFormatter.format(flightList.get(i).getDateTimeDepart()).equals(dateFormatter.format(search.getDateTimeDepart()))){
                    ...
                    // show seat 
                    JLabel seat = new JLabel("Seat left: " + (60 - flightList.get(i).getPassengers()) + " / 60 ",SwingConstants.RIGHT);
                    seat.setFont(new Font("Calibri",Font.BOLD,14));
                    ...
                    book.setPreferredSize(new Dimension(40,20));
                    c.fill = GridBagConstraints.HORIZONTAL; // fill up the row entirely
                    c.ipady = 5;   // Make the title tall; ipad: Inner padding (x or y)
                    c.ipadx = 5;
                    c.weightx = 0.0;    // Destance between row and column (in percentage max 1.0 min 0.0)
                    ...
                    book.addActionListener(new bookFlightListener(f));
                    pane.add(book,c);
                    row++;  // next row
                }
            }
    ...
    private class bookFlightListener implements ActionListener {
        ...
        public bookFlightListener(int f){
            flight_no_ref = f;
        }
        @Override
        public void actionPerformed(ActionEvent e) {
            dispose();
            bookingPage bookFlight = new bookingPage(flight_no_ref, flightList, frame);
        }
    }
}
